...
class StoneTowerStrategy(BuildingStrategy):
    ...
    def execute(self, agent, location):
        ...
        for i in range(10):
            # Check for pause
            while agent.state == AgentState.PAUSED: time.sleep(1)
            ...
            for dx in range(-1, 2):
                for dz in range(-1, 2):
                    ...
                    agent.mc.setBlock(x + dx, current_y, z + dz, block.STONE.id)
            
            time.sleep(0.5)

        # Add torches on top
        agent.mc.setBlock(x, y + 10, z, block.TORCH.id)
